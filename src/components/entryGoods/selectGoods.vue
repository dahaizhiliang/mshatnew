...
import { Checkpurchaselist } from '../../js/entryGoods/entryGoods.js'
export default {
    name:'SelectGoods',
    data(){
        return{
    ...
    mounted(){ 
        ...
        Checkpurchaselist(data)
            .then(res => {                
                res.data.purchaseList.forEach(ele => {
                    ele.check = false;
                    ele.fillOk = false;
                    ele.editUnitPrice = ele.PRICE;
                    ele.editNumber = 1;  
                })
                this.goodsList = res.data.purchaseList;
