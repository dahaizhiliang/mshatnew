<template>
    <div class="content">
        <ul class="box">
            ...
            <li>
                <div class="wrapper">
                    <span class="item-name">销售日期</span>
            ...
            <li>
                <div class="select-area link">
                    <div class="goods-msg">
                        <span class="item-name">销售商品</span>
        ...
        <ul class="box">
            <li>
                <div class="wrapper">
                    <span class="item-name">折扣价</span>
                    <input class="fill-input" type="number" v-model="discountPrice" placeholder="100">
                    <span class="right-text">%折</span>
                </div>                
            </li>
            <li>
                <div class="wrapper">
                    <span class="item-name">应收金额</span>
                    <input class="fill-input" type="number" v-model="receivableAmount"  placeholder="0.00">
                    <span class="right-text">元</span>
                </div>                
            </li>
            <li>
                <div class="wrapper">
                    <span class="item-name">实收金额</span>
...
import { SaveSellGoodsMsg } from '../../js/sellGoods/sellGoods.js';
...
export default {
    ...
    watch:{
        allGoodsTotal(val){//总金额
            this.realPayAmount = val;
            this.receivableAmount = val;
        },
        discountPrice(val){//折扣价
            this.receivableAmount = Number.parseFloat(this.allGoodsTotal * val / 100).toFixed(2);
            this.realPayAmount = Number.parseFloat(this.allGoodsTotal * val / 100).toFixed(2);
        },
        payWay(val){
            if(val == 0){
    ...
    methods:{
        ...
        save(){            
            ...
            let data = {
                buyer_booth_id:this.merchantsId,
                buyer_booth_name:this.merchants,
                seller_booth_id:JSON.parse(localStorage.getItem('buyerList'))[0].SHOP_BOOTH_ID,
                seller_booth_name:JSON.parse(localStorage.getItem('buyerList'))[0].BOOTH_NAME,
                ...
                .then(res => {
                    if(res.result){
                        ...
                        setTimeout(() => {
                            ...
                            this.$router.push({name:'SellConfirmPage',params:saveDate})
                        }, 1000);
                    }else{
                        Toast({
                            message: res.message,
